...
def run():
    ...
    if my_ip != route53_ip:
        logger.info("Will update IP in %s (%s) for %s from %s to %s", HOSTED_ZONE_DNS_NAME, hosted_zone_id, MY_DNS_NAME, route53_ip, my_ip)
        set_route53_ip(new_ip=my_ip, my_dns_name=MY_DNS_NAME, hosted_zone_id=hosted_zone_id, ttl=TTL)
    else:
        logger.info("IP in %s (%s) for %s (%s) matches, nothing to do", HOSTED_ZONE_DNS_NAME, hosted_zone_id, MY_DNS_NAME, my_ip)
...
def install():
    ...
    with open("/etc/systemd/system/py-unifi-route53-ddns.service.d/env.conf", "w") as env_fh:
        env_fh.write(systemd_override.format(akid=akid, access_key=access_key, zone_name=zone_name, host_name=host_name))
    logger.info('Done. Please run "systemctl start py-unifi-route53-ddns.timer" and "systemctl enable py-unifi-route53-ddns.timer".')
